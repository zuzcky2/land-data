...
class BoundaryCommand(AbstractCommand):
    ...
    def _process_sync_direct(self, location_type: str, label: str):
        ...
        try:
            self.message(f'🚀 [{label}] 직접 동기화를 시작합니다...', fg='green')
            ...
            self.message(f'-> 총 {meta.total}건 ({last_page}페이지) 대상 확인', fg='cyan')
            ...
            if last_page > 1:
                for page in range(2, last_page + 1):
                    ...
                    self.message(f'   ... {page}/{last_page} 페이지 진행 중', fg='blue')

            self.message(f'✅ [{label}] 모든 데이터 쓰기 완료', fg='green')
    ...
    def _process_sync_hierarchy(self, parent_type: str, current_type: str, label: str):
        ...
        try:
            self.message(f'📂 [{label}] 계층적 동기화 시작 (Source: {parent_type.upper()})', fg='yellow')
            ...
            self.message(f'✅ [{label}] 동기화 완료: 총 {total_count}개 데이터 저장', fg='green')
