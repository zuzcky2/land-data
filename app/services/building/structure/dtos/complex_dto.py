...
class ComplexDto(MongoModel):
    ...
    # K-APT 단지 정보 (아파트 단지 전용)
    kapt_code: Optional[str] = Field(None, title='K-APT 단지코드 (kaptCode)')
    kapt_name: Optional[str] = Field(None, title='K-APT 단지명')
    heating_type: Optional[str] = Field(None, title='난방방식 (예: 개별난방, 중앙난방)')
    area_60_count: Optional[int] = Field(None, title='전용면적 60㎡ 이하 세대수')
    area_85_count: Optional[int] = Field(None, title='전용면적 60~85㎡ 세대수')
    area_135_count: Optional[int] = Field(None, title='전용면적 85~135㎡ 세대수')
    area_136_count: Optional[int] = Field(None, title='전용면적 135㎡ 초과 세대수')
    management_company: Optional[str] = Field(None, title='관리회사명')
    elevator_count: Optional[int] = Field(None, title='엘리베이터 수')
    parking_count_kapt: Optional[int] = Field(None, title='주차대수 (K-APT 기준)')
    subway_station: Optional[str] = Field(None, title='인근 지하철역명')
    subway_line: Optional[str] = Field(None, title='지하철 노선명')
    bus_walktime: Optional[int] = Field(None, title='버스정류장 도보시간(분)')
    subway_walktime: Optional[int] = Field(None, title='지하철역 도보시간(분)')
    convenient_facility: Optional[str] = Field(None, title='편의시설')
    education_facility: Optional[str] = Field(None, title='교육시설')
    welfare_facility: Optional[str] = Field(None, title='복지시설')
    ev_charger_count: Optional[int] = Field(None, title='전기차 충전기 수 (지상+지하)')
